...
- `/` — главная: сразу глобус, без intro
- `/about` — о проекте + цитата Raye
...
Компонент [components/StoryOverlay.tsx](components/StoryOverlay.tsx). Обёртка над `StoryPopup` с fade-in/out **600ms** (в дыхательном темпе). Закрытие по ESC, backdrop-click, кнопке ×.

На мобильном — bottomsheet (`items-end`), на десктопе — центральная карточка (`sm:items-center`).

Содержимое попапа (`StoryPopup.tsx`):
- Верхний бейдж: `getting through` (тёплый) или `still in the dark` (холодный)
- Цитата: serif italic в кавычках, `rgba(255,255,255,0.85)`
- Если есть coped — тонкая линия + label `What keeps them going:`
- Внизу рандомная фраза из `lib/phrases.ts` (seed по `story.id`, чтобы при повторном открытии была та же)
...
- Fade попапов — 600ms (дыхательный темп)
...
| POST | `/api/stories` | Создать: принимает только `{text, coped, lang}`. Координаты вычисляет сам |
...
**Known limitation.** Текущая реализация (`lib/store.ts`) хранит counter в `globalThis.Map` — выживает между запросами в dev Next.js process, но **не в Vercel serverless** (каждая cold start — свой процесс).

**Для продакшена** нужен либо:
1. Таблица `submissions` в Supabase с `ip_hash` + `created_at` + SQL `count(*) where created_at > now() - interval '1 hour'`
2. Upstash Redis / Vercel KV

Пока не сделано — rate limit в prod фактически не работает (считает только в рамках одного инстанса). Добавить перед серьёзным запуском.
...
Показываются внизу каждого попапа, тихо (`opacity ~0.35`). По 10 штук EN + RU. Захардкожены в [lib/phrases.ts](lib/phrases.ts). Seed по `story.id` — стабильный выбор.
...
/app
  ...
  /about/page.tsx            манифест + цитата Raye
  /api/stories/route.ts      GET/POST
  /layout.tsx                фонты, metadata
  /opengraph-image.tsx       динамический OG
  /globals.css               @theme + breathing keyframes
...
/components
  ...
  /StorySparks.tsx           Sprite'ы + custom raycasting
  /StoryOverlay.tsx          fade-600ms обёртка
  /StoryPopup.tsx            содержимое попапа
  /AddStoryModal.tsx         форма без location UI и без выбора языка
  /Header.tsx                логотип + About
  /EncouragingFooter.tsx     random phrase
...
/lib
  ...
  /phrases.ts                10 EN фраз
  /seed.ts                   12 историй (fallback, не используется с Supabase)
  /store.ts                  in-memory fallback + rate limit (local only)
  /stopwords/en.ts
...
## Статус (на 17 апреля 2026)

**Готово:**
- ✅ Глобус, Антарктика через h3+patch
- ✅ Shader fire + particles
- ✅ Искры, raycasting, StoryOverlay, fade 600ms
- ✅ AddStoryModal без location-UI и без выбора языка, autogeoloc по IP
- ✅ Sharp entry — сайт открывается сразу глобусом, без intro
- ✅ Touch-aware подсказки через Tailwind 4
- ✅ Mobile responsive (bottomsheet'ы)
- ✅ Supabase подключён (новые sb_publishable/secret ключи)
- ✅ Seed 12 историй в БД
- ✅ Groq moderation (crisis-aware prompt) — проверена на 6 reality-кейсах
- ✅ SEO / OG / favicon
- ✅ English-only (RU удалён из UI)

**Известные ограничения:**
- ⚠️ Rate limit работает только in-memory (не выживёт на Vercel serverless)
- ⚠️ Нет мониторинга ошибок (Sentry / etc.)
- ⚠️ `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` зарезервирован, но код его не использует (все запросы через server с secret key)

**Не сделано:**
- ❌ Деплой на Vercel
- ❌ Rate limit в БД / KV (нужен перед публичным релизом)
